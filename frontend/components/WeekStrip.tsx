...
export default function WeekStrip({ selectedDate, onSelectDate, itemDates }: Props) {
  ...
  return (
    <div className="flex items-center gap-1 border-b border-slate-800 bg-slate-900/50 px-4 py-3">
      <button
        type="button"
        onClick={() => shiftWeek(-1)}
        ...
        return (
          <button
            ...
            className={`flex flex-1 flex-col items-center rounded-lg py-2 transition ${
              isSelected ? 'bg-primary-500 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
            }`}
          >
